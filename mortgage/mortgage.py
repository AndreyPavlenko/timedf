...
def run_pd_workflow(quarter, year, perf_file, fragment_size):
    ...
    acquisition_path = os.path.join(data_directory, "acq", "Acquisition_" + str(year) + "Q" + str(quarter) + ".txt")
...
"""
    create_table_names = create_table_names_temlate.replace("##FRAGMENT_SIZE##", str(fragment_size))
...
def create_ever_features():
    ...
    con.execute('CREATE TABLE ever_30temp AS SELECT everdftemp1.loan_id AS loan_id,everdftemp1.current_loan_delinquency_status AS current_loan_delinquency_status ,ever_30.current_loan_delinquency_status AS ever_30 FROM ever_30  LEFT JOIN everdftemp1 ON ever_30.loan_id = everdftemp1.loan_id;');
...
def create_12_mon_features():
  ...
  #tmpdf['josh_months'] = tmpdf['timestamp_year'] * 12 + tmpdf['timestamp_month']
     con.execute('CREATE TABLE delinq_12 AS SELECT FLOOR((josh_months-24000-'+string+')/12) AS josh_mody_n,loan_id from tmpdf;');
...
parser.add_argument('-fs', dest="fragment_size", action='append', type=int, help="Fragment size to use for created table. Multiple values are allowed and encouraged.")
...
for fs in args.fragment_size:
    for iii in range(1, args.iterations + 1):
        ...
        for quarter in range(0, args.df):
            ...
            for f in files:
                dataframe, exec_time = run_pd_workflow(year = year, quarter = (quarter % 4 + 1),
                                                       perf_file = str(f), fragment_size = fs)
