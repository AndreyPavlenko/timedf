...
class MortgagePandasBenchmark:
    ...
    def run_cpu_workflow(self, quarter=1, year=2000, perf_file="", **kwargs):
        ...
        acq_gdf = self.cpu_load_acquisition_csv(acquisition_path=self.acq_data_path + "/Acquisition_"
    ...
    def cpu_load_acquisition_csv(self, acquisition_path, **kwargs):
        ...
        return self._parse_dtyped_csv(acquisition_path, dtypes, names=cols, delimiter='|', index_col=False)
    ...
    def combine_joined_12_mon(self, joined_df, testdf, **kwargs):
        ...
        t0 = default_timer()
        df = joined_df.merge(testdf, how='left', on=['loan_id', 'timestamp_year', 'timestamp_month'])
        t1 = default_timer()
        self.t_merge += t1-t0
    ...
    def last_mile_cleaning(self, df, **kwargs):
        drop_list = [
            ...
            'monthly_reporting_period', 'last_paid_installment_date', 'maturity_date',
            'upb_12','zero_balance_effective_date','foreclosed_after', 'disposition_date','timestamp'
        ]
...
def main():
    ...
    parser.add_argument('-algo', choices=["xgb", "daal"], default="xgb", help="xgb : xgb; daal: daal")
    parser.add_argument("-pandas_mode",
                        choices=["pandas", "modin_on_ray", "modin_on_dask"], default="pandas",
                        help="Specifies which version of Pandas to use: plain Pandas, Modin runing on Ray or on Dask")
    parser.add_argument("-ray_tmpdir", default="/tmp",
                        help="Location where to keep Ray plasma store. It should have enough space to keep -ray_memory")
    parser.add_argument("-ray_memory", default=200*1024*1024*1024,
