...
python3 run_omnisci_benchmark.py --env_name ${ENV_NAME} --env_check True --save_env True --mode synthetic                      \
                                 ...
                                 --import-table-name sort_benchmark --label sort_test                                          \
                                 ...
                                 --synthetic-query Sort --num-fragments 10                                                     \
                                 -commit ${BUILD_REVISION}                                                                     \
                                 -db-server ${DATABASE_SERVER_NAME} -db-name "${DATABASE_NAME}" -db-table sortbench            \
