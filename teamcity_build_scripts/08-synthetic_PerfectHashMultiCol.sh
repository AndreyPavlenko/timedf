...
python3 run_omnisci_benchmark.py --env_name ${ENV_NAME} --env_check True --save_env True --mode synthetic                           \
                                 ...
                                 --import-table-name perfect_hash_multi_col_benchmark --label perfect_hash_multi_col_test           \
                                 ...
                                 --synthetic-query PerfectHashMultiCol --num-fragments 10                                           \
                                 -commit ${BUILD_REVISION}                                                                          \
                                 -db-server ${DATABASE_SERVER_NAME} -db-name "${DATABASE_NAME}" -db-table perfecthashmulticolbench  \
