...
def main():
    ...
    parser.add_argument('--no-ht', default=False, action='store_true',
                        help='do not use Hyper Threading')
    parser.add_argument('--n0-only', default=False, action='store_true',
                        help='operate within single node')
    parser.add_argument('--each', default=False, action='store_true',
                        help='print each number without ranges')
    parser.add_argument('--total', default=False, action='store_true',
                        help='print total number of selected cpus')
    ...
    with tempfile.SpooledTemporaryFile(mode='w+') as tmp, Popen("lscpu -p", shell=True, stdout=PIPE) as pipe:
        for line in iter(pipe.stdout.readline, b''):
            l = line.decode()
            if(l.startswith('#') and not l.startswith('# CPU')):
                continue
            tmp.write(l)
        tmp.seek(0)
        df = pd.read_csv(tmp)
