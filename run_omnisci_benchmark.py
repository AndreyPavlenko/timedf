...
optional.add_argument("-sq", "--synthetic-query", choices=['BaselineHash', 'MultiStep',
                                                           'NonGroupedAgg', 'PerfectHashMultiCol',
                                                           'PerfectHashSingleCol', 'Sort'],
...
optional.add_argument("-c", "--table-schema-file", dest="table_schema_file",
                      help="Path to local file with CREATE TABLE sql statement for "
                           "the import table.")
...
optional.add_argument("-db-user", default="",
...
optional.add_argument("-db-pass", default="omniscidb",
                      help="Password to use to connect to MySQL database.")
optional.add_argument("-db-name", default="omniscidb",
                      help="MySQL database to use to store benchmark results.")
optional.add_argument("-db-table", help="Table to use to store results for this benchmark.")

optional.add_argument("-commit", default="1234567890123456789012345678901234567890",
...
if args.mode == 'synthetic':
    if args.synthetic_query is None or args.num_synthetic_fragments is None \
            or args.fragment_size is None:
        print("For synthetic type of benchmark the following parameters are mandatory: "
              "--synthetic-query, --num-fragments and --fragment-size.")
        sys.exit(3)
    datafiles = 0
    results_file_name = os.path.join(args.benchmarks_path, 'synthetic_results', args.label,
                                     'CPU', 'Benchmarks', args.synthetic_query + '.json')
    import_cmdline = None
    ...
    if args.import_file is None or args.table_schema_file is None or args.queries_dir is None:
        print("For dataset type of benchmark the following parameters are mandatory: --import-file,"
...
try:
    ...
    with open(args.report, "w") as report:
        print("datafiles,fragment_size,query,query_exec_min,query_total_min,query_exec_max,"
